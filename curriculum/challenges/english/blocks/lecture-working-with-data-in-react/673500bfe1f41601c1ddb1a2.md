...
Notice the double curly braces `{{ backgroundColor: "#007BFF" }}`. The outer curly braces `{}` indicate that we are using a JavaScript expression in JSX. The inner curly braces `{}` define a JavaScript object literal. When writing inline styles in React, you must pass a JavaScript object, not a CSS string.
